...
#define PRE_ALLOC_SIZE		(80 * 1024 * 1024)
...
static int do_newinst_cmd(string type, string name, string arg) {
    ...
    void *module = modules["hal_mod"];
    if(!module) {
        rtapi_print_msg(RTAPI_MSG_ERR, 
                "newinst: hal_mod is required, but not loaded\n");
        return -1;
    }
...
static int do_load_cmd(string name, vector<string> args) {
    ...
    if(w == NULL) {
        char what[LINELEN+1];
        snprintf(what, LINELEN, "%s/lib%s.so", EMC2_RTLIB_DIR, name.c_str());
...
static int master(int fd, vector<string> args) {
    ...
    do_load_cmd("hal_mod", vector<string>()); instance_count = 0;
    if(args.size()) { 
...
void backtrace_handler(int sig, siginfo_t *, void *)
{
    void *bt[32];
    int nentries;
...
int main(int argc, char **argv) 
{
    ...
    snprintf(what, LINELEN, "%s/libethercat.so", EMC2_RTLIB_DIR);
    handle = dlopen(what, RTLD_LAZY | RTLD_GLOBAL );
    if (!handle) {
        handle = dlopen("/opt/etherlab/lib/libethercat.so", RTLD_LAZY | RTLD_GLOBAL );
        if (!handle) {
            handle = dlopen("/usr/lib/libethercat.so", RTLD_LAZY | RTLD_GLOBAL );
            if (!handle) {
                rtapi_print_msg(RTAPI_MSG_WARN,
                        "EtherCAT Library OpenFail 3 \n");
            }
        }
    }

    snprintf(what, LINELEN, "%s/libmxml.so", EMC2_RTLIB_DIR);
    handle = dlopen(what, RTLD_LAZY | RTLD_GLOBAL );
    if (!handle) {
        handle = dlopen("/usr/lib/libmxml.so", RTLD_LAZY | RTLD_GLOBAL );
        if (!handle) {
            rtapi_print_msg(RTAPI_MSG_WARN,
                    "XML Library OpenFail 3 \n");
        }
    }
