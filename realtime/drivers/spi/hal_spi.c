...
int rtapi_app_main(void)
{
    ...
    /* export functions */
    rtapi_snprintf(buf, HAL_NAME_LEN, "spi.read");
    rv1 = hal_export_funct(buf, read_all, &(bus_array[busnum]), 1, 0, comp_id);
    if (rv1 != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SPI: ERROR: read funct export failed\n");
        rv = -1;
        /* skip to next bus */
    }

    rtapi_snprintf(buf, HAL_NAME_LEN, "spi.write");
    rv1 = hal_export_funct(buf, write_all, &(bus_array[busnum]), 1, 0, comp_id);
    if (rv1 != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SPI: ERROR: write funct export failed\n");
        rv = -1;
        /* skip to next bus */
    }
    ...
    //rtapi_print_msg(RTAPI_MSG_INFO, "SPI: driver installed\n");
    hal_ready(comp_id);
