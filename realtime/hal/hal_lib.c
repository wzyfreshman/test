...
int hal_create_thread(const char *name, unsigned long per_nsec, int uses_fp, int cpu_id)
{
    int next, cmp, prev_priority;
    int retval, n;
    hal_thread_t *new, *tptr;

    long period_nsec = per_nsec - per_nsec/4;
    long prev_period, curr_period;

    rtapi_print_msg(RTAPI_MSG_ERR,
        "HAL: hal_create_thread Limited %d to %d\n",per_nsec,period_nsec);
...
/*! \todo Another #if 0 */
    ...
    prev_priority = rtapi_prio_highest()-2;//ghf-2014/12/2
    ...
    new->priority = rtapi_prio_next_higher(prev_priority);
...
hal_comp_t *halpr_find_comp_by_name(const char *name)
{
    ...
    next = hal_data->comp_list_ptr;
    //rtapi_print_msg(RTAPI_MSG_ERR,"next=%d\n",next);
    while (next != 0) {
	comp = SHMPTR(next);
...
int rtapi_app_main(void)
{
    ...
    printf("Setting up /tmp/halready.flag\n");
    system("touch /tmp/halready.flag");
    return 0;
}
