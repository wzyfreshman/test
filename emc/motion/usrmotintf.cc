...
static emcdata_struct *motData = 0;
...
//    static double usetime;
//    usetime = etime();
    *emcmotCommand = *c;
//    printf("now use time %f\n",etime()-usetime);
    /* poll for receipt of command */
    /* set timeout for comm failure, now + timeout */
    end = etime() + EMCMOT_COMM_TIMEOUT;//1秒后超时

    /*rcs_print( "emcmotCommand->commandNum = %d, emcmotCommand->command = %3d emcmotCommand->head=%d\n",
            emcmotCommand->commandNum,
            emcmotCommand->command,emcmotCommand->head);*/

    /* now check to see if it got it */
    while (etime() < end) {
        /* update status */
        if(emcmotCommand->command == EMCMOT_DOUBLEZ) {//ghf-2014/7/30
            if (( usrmotReadEmcmotStatus(&s) == 0 ) && ( s.commandNumEcho == commandNum )) {//第二个括号确保进去了emcmotCommandHandler，第一个确保执行到了末尾
                /* now check emcmot status flag */
                if (s.commandStatus == EMCMOT_COMMAND_DOUBLEZOK) {
                    if(s.motion_state != EMCMOT_MOTION_COORD) {
                        printf("manual change z\n");
                        return EMCMOT_COMM_OK;
                    } else if(s.changezend > 0 || s.changingz == 1) {
                        printf("auto change z\n");
                        return EMCMOT_COMM_OK;
                    }
                } else {
                    rcs_print("USRMOT: ERROR: SEND EMCMOT_DOUBLEZ,s.commandStatus=%d,s.head=%d,s.tail=%d,s.heartbeat=%d\n",s.commandStatus,s.head,s.tail,s.heartbeat);
                    rcs_print("USRMOT: ERROR: SEND EMCMOT_DOUBLEZ,s.commandNumEcho=%d\n",s.commandNumEcho);
                    rcs_print("USRMOT: ERROR: SEND EMCMOT_DOUBLEZ,commandNum=%d\n",commandNum);
                    rcs_print("USRMOT: ERROR: SEND EMCMOT_DOUBLEZ,emcmotCommand->command=%d\n",emcmotCommand->command);
                    return EMCMOT_COMM_ERROR_COMMAND;
                }
            }
        }
        else if((emcmotCommand->command == EMCMOT_SET_LINE)||(emcmotCommand->command == EMCMOT_SET_CIRCLE)) {//ghf-2014/10/20
            if (( usrmotReadEmcmotStatus(&s) == 0 ) && ( s.commandNumEcho == commandNum )) {//第二个括号确保进去了emcmotCommandHandler，第一个确保执行到了末尾
                /* now check emcmot status flag */
                if (s.commandStatus == EMCMOT_COMMAND_LINECYCLEOK) {
                    return EMCMOT_COMM_OK;
                } else {
                    rcs_print("USRMOT: ERROR: SEND SETLINE OR SETCYCLE,emcmotCommand->command=%d\n",emcmotCommand->command);
                    return EMCMOT_COMM_ERROR_COMMAND;
                }
            }
        }
...
int usrmotReadEmcData(emcdata_struct *s)
{
    /* copy status struct from shmem to local memory */
    return 0;
    static double usetime = etime();
    usetime = etime();
    motData->locksig = 1;
    memcpy(s, motData, sizeof(emcdata_struct));
    motData->locksig = 0;
}
...
int usrmotReadEmcmotStatus(emcmot_status_t * s)
{
    ...
    do {
        ...
        if (s->head == s->tail) { //emcmotCommandHandler结尾时，才会执行emcmotStatus->tail = emcmotStatus->head;
            /* head and tail match, done */
            if(s->commandEcho == EMCMOT_DOUBLEZ) { //ghf-2014/8/8
                if(s->commandStatus != EMCMOT_COMMAND_DOUBLEZOK){
                    return -5;
                }
            }
            else if((s->commandEcho == EMCMOT_SET_LINE)||(s->commandEcho == EMCMOT_SET_CIRCLE)) { //ghf-2014/10/20
                if(s->commandStatus != EMCMOT_COMMAND_LINECYCLEOK){
                    return -6;
                }
            }
...
int usrmotInit(const char *modname)
{
    ...
    //    "\n\nusrmotintf: modname=%s SHMEM_KEY=%d\n",modname,SHMEM_KEY);
    ...
    rtapi_print_msg(RTAPI_MSG_ERR,"SHMEM_KEY=%d,mot_comp_id=%d\n",SHMEM_KEY,module_id);
    //rtapi_print_msg(RTAPI_MSG_ERR,"sizeof(emcmot_status_t)=%d\n",sizeof(emcmot_status_t));
    //rtapi_print_msg(RTAPI_MSG_ERR,"sizeof(emcmot_config_t)=%d\n",sizeof(emcmot_config_t));
    //rtapi_print_msg(RTAPI_MSG_ERR,"sizeof(emcmot_internal_t)=%d\n",sizeof(emcmot_internal_t));
    //rtapi_print_msg(RTAPI_MSG_ERR,"sizeof(emcmot_error_t)=%d\n",sizeof(emcmot_error_t));
    //rtapi_print_msg(RTAPI_MSG_ERR,"sizeof(emcmot_debug_t)=%d\n",sizeof(emcmot_debug_t));
    ...
    rtapi_print_msg(RTAPI_MSG_ERR,"111");
    ...
    rtapi_print_msg(RTAPI_MSG_ERR,"retval=%d\n",retval);

    /* get shared memory block from RTAPI */
    data_id = rtapi_shmem_new(54321, module_id, sizeof(emcdata_struct));
    if (data_id < 0) {
    fprintf(stderr,
        "usrmotintf: ERROR: could not open shared memory data\n");
    rtapi_exit(data_id);
    return -1;
    }
    printf("now I get the usrmot------------- data_id:%d    shmem_id:%d\n",data_id,shmem_id);
    /* get address of shared memory area */
    retval = rtapi_shmem_getptr(data_id, (void **) &motData);
    if (retval < 0) {
    rtapi_print_msg(RTAPI_MSG_ERR,
        "usrmotintf: ERROR: could not access shared memory data\n");
    rtapi_exit(data_id);
    return -1;
    }
...
int usrmotLoadComp(int joint, const char *file, int type)
{
    ...
    int i=0;
    char nombuf[LINELEN],fwdbuf[LINELEN],revbuf[LINELEN];
    ...
    while (!feof(fp)) {
        ...
        memset(nombuf,0,sizeof(nombuf));
        memset(fwdbuf,0,sizeof(fwdbuf));
        memset(revbuf,0,sizeof(revbuf));
        if (NULL == fgets(buffer, LINELEN, fp)) {//LINELEN=255
            break;
        }
        ...
        for(i=0;i<LINELEN;i++) {
            ...
            if((buffer[i] > '9' || buffer[i] < '0') && buffer[i] != '.' && buffer[i] != '-') {
                //printf("count=%d,buffer[%d]=%c\n",count,i,buffer[i]);
                if(l > 0) {
                    l = 0;
                    count++;
                }
            } else {
                //printf("11count=%d,buffer[%d]=%c\n",count,i,buffer[i]);
                if(0 == count) {
                    nombuf[l] = buffer[i];
                    l++;
                } else if(1 == count) {
        ...
        //printf("nombuf=%s,revbuf=%s,fwdbuf=%s\n",nombuf,revbuf,fwdbuf);

        if((nombuf[0]!=0)&&(revbuf[0]!=0)&&(fwdbuf[0]!=0)) {
            sscanf(nombuf, "%lf", &nom);
            sscanf(revbuf, "%lf", &rev);
            sscanf(fwdbuf, "%lf", &fwd);
            //printf("%lf,%lf,%lf\n", nom, rev, fwd);
