...
int hal_ppmc10( uint32 SK[32], uint8 key[8] )
{
    int i;
    uint32 X, Y, T;
...
void hal_ppmc8( uint32 SK[32], uint8 input[8], uint8 output[8] )
{
    uint32 X, Y, T;
...
{
    hal_ppmc8( ctx->esk, input, output );
}

void hal_ppmc_3( des_context *ctx, uint8 input[8], uint8 output[8] )
{
    hal_ppmc8( ctx->dsk, input, output );
}
...
int hal_ppmc_4( des3_context *ctx, uint8 key1[8], uint8 key2[8] )
{
    ...
    hal_ppmc10( ctx->esk     , key1 );
    hal_ppmc10( ctx->dsk + 32, key2 );
...
void hal_ppmc9( uint32 SK[96], uint8 input[8], uint8 output[8] )
{
    uint32 X, Y, T;
...
int main( void )
{
    ...
    for( m = 0; m < 2; m++ )
    {
        ...
        for( n = 0; n < 3; n++ )
        {
            ...
            {
                case 0:
                    hal_ppmc_1( &ctx, DES3_keys[0] );
                    break;

                case 1:
                    hal_ppmc_4( &ctx3, DES3_keys[0],
                                           DES3_keys[1] );
                    break;

                case 2:
                    hal_ppmc_5( &ctx3, DES3_keys[0],
                                           DES3_keys[1],
                                           DES3_keys[2] );
                    break;
            }
            ...
            for( i = 0; i < 10000; i++ )
            {
                if( n == 0 )
                {
                    if( m == 0 ) hal_ppmc_2( &ctx, buf, buf );
                    if( m == 1 ) hal_ppmc_3( &ctx, buf, buf );
                }
                else
                {
                    if( m == 0 ) hal_ppmc_6( &ctx3, buf, buf );
                    if( m == 1 ) hal_ppmc_7( &ctx3, buf, buf );
                }
            }
