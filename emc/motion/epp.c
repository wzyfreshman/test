...
#define des_set_key  hal_ppmc_1
#define des_encrypt  hal_ppmc_2
#define des_decrypt  hal_ppmc_3
...
int LKT4200_reset(unsigned char rstinfo[8]){
    ...
    send_cmd[1] = 0;
    send_cmd[2] = 0;
    ret = sendLKcmd(send_cmd,3,localread,&LenOfRes) ;
...
static int  write8Bytedown(unsigned char *addrin,unsigned char *data) {
    ...
    for(  i = 0; i < 8; i++ )
        send_cmd[3 + i] = data[i];
    sendLKcmd(send_cmd,11,ret_cmd,&LenOfRes) ;

//    int j=0,k = LenOfRes;
//    for(j=0;j<k;j++) {
//        printf("%.2X",ret_cmd[j]);
//    }
//    printf(" write8Bytedown ret %d\n",LenOfRes);
