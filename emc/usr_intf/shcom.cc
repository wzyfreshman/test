...
#define EMC_COMMAND_DELAY   0.01	// how long to sleep between checks,ghf-2014/3/20 
...
int sendSpindleFeed(double val)
{
    EMC_SPINDLE_SPEED emcSpindlespeed;
    emcSpindlespeed.speed = val;
    emcSpindlespeed.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcSpindlespeed);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendParaFlush()
{
    EMC_TASK_FLUSH_PAR emcFlushparmsg;
    emcFlushparmsg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcFlushparmsg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendSpindleForward(double val)
{
...
int sendUnHome(int axis,int flag)
{
...
int sendChoiceDefaultFeed(double dffeed) {
    EMC_TRAJ_SET_DEFAULT_FEED emc_set_default_feed;
    emc_set_default_feed.serial_number = ++emcCommandSerialNumber;
    emc_set_default_feed.feed = dffeed;
    emcCommandBuffer->write(emc_set_default_feed);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendProgramRun(int line,int endline)
{
    ...
    emc_task_plan_run_msg.endline = endline;
...
int sendProReadOnly(int flag)
{
    EMC_PRO_VERIFY_CMD emcProverifymsg;
    emcProverifymsg.verify_flag =  flag;
    emcProverifymsg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcProverifymsg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendResetcmd()
{
    EMC_RESET_CMD emcResetmsg;
    emcResetmsg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcResetmsg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendDoubleZcmd(int axis , int m6_flags ,double *z_diff)
{
    EMC_DOUBLEZ_CMD emcDoubleZ_msg;
    emcDoubleZ_msg.serial_number = ++emcCommandSerialNumber;
    emcDoubleZ_msg.set_which_z = 0;
    emcDoubleZ_msg.q_flag = axis;//ghf-2014/4/2
    emcDoubleZ_msg.m6_flag = m6_flags;
    ...
    sendSpindleOff();
    for(int i=0;i++;i<9)
        emcDoubleZ_msg.z_diff[i] = z_diff[i];
    emcCommandBuffer->write(emcDoubleZ_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
//ghf-2014/3/11
int sendJogwheels_guidecmd(unsigned char status)
{
    EMC_JOGWHEELS_GUIDE_CMD emcJogwheels_guide_msg;
    emcJogwheels_guide_msg.serial_number = ++emcCommandSerialNumber;
    emcJogwheels_guide_msg.jogwheels_guide = status;
    emcCommandBuffer->write(emcJogwheels_guide_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
//ghf-2014/3/12
int sendSetgmodecmd(TGMODE *gmode0, TGMODE *gmode1)
{
    EMC_SETGMODE_CMD emcSetgmode_msg;
    printf("in the sendSetgmodecmd\n");
    emcSetgmode_msg.serial_number = ++emcCommandSerialNumber;    
    memcpy(&(emcSetgmode_msg.gmode0),gmode0,sizeof(emcSetgmode_msg.gmode0));
    memcpy(&(emcSetgmode_msg.gmode1),gmode1,sizeof(emcSetgmode_msg.gmode1));
    emcCommandBuffer->write(emcSetgmode_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
//ghf-2014/3/17
int sendSetlightcmd(unsigned char setlight)
{
    EMC_SETLIGHT_CMD emcSetlight_msg;    
    printf("now Set light %d\n",setlight);
    emcSetlight_msg.serial_number = ++emcCommandSerialNumber;    
    emcSetlight_msg.setlight = setlight;    
    emcCommandBuffer->write(emcSetlight_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
//ghf-2014/3/28
int sendEncryptcmd(char* encryptbuf)
{
    EMC_ENCRYPT_CMD emcEncrypt_msg;    
    emcEncrypt_msg.serial_number = ++emcCommandSerialNumber;    
    memcpy(emcEncrypt_msg.encryptbuf,encryptbuf,sizeof(encryptbuf));    
    emcCommandBuffer->write(emcEncrypt_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
//ghf-2014/5/6
int sendFiltertimecmd(char* filtertimebuf)
{
    EMC_FILTERTIME_CMD emcFiltertime_msg;    
    emcFiltertime_msg.serial_number = ++emcCommandSerialNumber;    
    memcpy(emcFiltertime_msg.filtertimebuf,filtertimebuf,sizeof(filtertimebuf));    
    emcCommandBuffer->write(emcFiltertime_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendAutoRepeat(bool val)
{
    EMC_TASK_AUTOREPEAT_PAR emcAutorepeatmsg;
    emcAutorepeatmsg.enable = val;
    emcAutorepeatmsg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcAutorepeatmsg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendMdiCmd(const char *mdi,int flag)
{
    EMC_TASK_PLAN_EXECUTE emc_task_plan_execute_msg;
    emc_task_plan_execute_msg.reset_flag = flag;
...
int sendMdiCmd(const char *mdi,int *ret,int flag)
{
    EMC_TASK_PLAN_EXECUTE emc_task_plan_execute_msg;
    emc_task_plan_execute_msg.reset_flag = flag;
    strcpy(emc_task_plan_execute_msg.command, mdi);
    emc_task_plan_execute_msg.serial_number = ++emcCommandSerialNumber;
    if(ret!=NULL)
        *ret = emc_task_plan_execute_msg.serial_number;
    emcCommandBuffer->write(emc_task_plan_execute_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
...
int sendSetlefttime(int val)
{
    EMC_TASK_SET_LEFTTIME emc_lefttime_msg;
    emc_lefttime_msg.lefthours = val;

    emc_lefttime_msg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emc_lefttime_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendStartLine(int line)
{
    EMC_TASK_SET_STARTLINE emc_setline_msg;
    emc_setline_msg.startline = line;

    emc_setline_msg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emc_setline_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
    return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendWaitSignal(int signal)
{
    EMC_TASK_SET_SIGNAL emc_setsignal_msg;
    emc_setsignal_msg.signal = signal;
    emc_setsignal_msg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emc_setsignal_msg);
    //if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    //} else if (emcWaitType == EMC_WAIT_DONE) {
    //return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendSetParameter(int index,double val) {
    EMC_TASK_SET_PARAMETER emc_setparameter_msg;
    emc_setparameter_msg.index = index;
    emc_setparameter_msg.val = val;
    emc_setparameter_msg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emc_setparameter_msg);
    //if (emcWaitType == EMC_WAIT_RECEIVED) {
    return emcCommandWaitReceived(emcCommandSerialNumber);
    //} else if (emcWaitType == EMC_WAIT_DONE) {
    //return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendSetDIO(int io,int flag)
{
    EMC_MOTION_SET_DOUT emcSetdiomsg;
    emcSetdiomsg.index = io;
    emcSetdiomsg.start = flag;		// startvalue = 1
    emcSetdiomsg.end = flag;		// endvalue = 1, means it doesn't get reset after current motion
    emcSetdiomsg.now = 1;		// immediate, we don't care about synching for AUX
    emcSetdiomsg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emcSetdiomsg);
    return emcCommandWaitReceived(emcCommandSerialNumber);

}

int sendSetExternIO(int io,int flag)
{
    EMC_MOTION_SET_EXTERN_OUT setIo;
    setIo.index = io;
    setIo.status = flag;
    setIo.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(setIo);
    return emcCommandWaitReceived(emcCommandSerialNumber);
}

int sendSetAnalogyIO(int io,int flag)
{
    EMC_MOTION_SET_AOUT setIo;
    setIo.index = io;
    setIo.start = flag;		// startvalue = 1
    setIo.end = flag;		// endvalue = 1, means it doesn't get reset after current motion
    setIo.now = 1;		// immediate, we don't care about synching for AUX
    setIo.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(setIo);
    return emcCommandWaitReceived(emcCommandSerialNumber);
}

int sendMultiFunction(int type,int ival,double dval) {
    EMC_TRAJ_MULTI_FUNCTION msg;
    msg.type = type;
    msg.intpar = ival;
    msg.doublepar = dval;
