...
//diagnostics("~conrnervelocity in: s2->Id=%d,s2->angle=%d\n",s2->ID,(int)(s2->angle*1000*180/3.1415926));
 ...
 //   if (s1->ID>506 && s1->ID<510)
   //  printf(" sqGiveCornerVelocity()4:s1->id=%d,s2->id=%d,vmax=%f,maxAcc_nLine=%f\n",s1->ID,s2->ID,vmax,maxAcc_nLine);
    ...
    if(fabs(s1->Kinematics5Axis.KnifeLength - s2->Kinematics5Axis.KnifeLength) > 1e-6  || s1->Kinematics5Axis.mode != s2->Kinematics5Axis.mode)
    {
        s2->angle = 0.0;
        return 0.0;
    }
    ...
    if (s2->type == SQ_CIRCULAR){
        ...
        coefficeint2[2] *= v2.z;
            ...
            if(sq->abGoAlongXYZ !=0){

                    double max_coefficient0 = fabs(coefficeint1[0]);
                    double max_coefficient1 = fabs(coefficeint2[0]);
                    //计算xyz轴是否有变动
                    for(i=1;i<3;i++)
                    {
                        max_coefficient0 = max(max_coefficient0,fabs(coefficeint1[i]));
                        max_coefficient1 = max(max_coefficient1,fabs(coefficeint2[i]));
                    }
                    /**********************************************************************/
                    //如果xyz轴中，s1,s2有数据变化，则其他轴随动
                    if(max_coefficient0>1e-6 && max_coefficient1>1e-6)
                    {
                        //s1,s2数据都有变化
                        maxdiff = fabs(axis[0]);
                        for (i=1;i<3;i++)
                   ...
                   //   printf("corner:s1.id=%d,s1.type=%d, s1 not zero,s2 not zero\n",s1->ID,s1->type);
                    }else if(max_coefficient0>1e-6 && max_coefficient1<=1e-6)
                    {
                    //xyz轴中s1轴动，s2轴不动
                     //   printf("corner:s1.id=%d,s1.type=%d, s1 not zero,s2 zero,v2.x=%f,v2.y=%f,v2.z=%f,coe=%f\n",s1->ID,s1->type,v2.x,v2.y,v2.z,s2->length_coefficient[2]);
                        return 0.0;
                    }else if(max_coefficient0<=1e-6 && max_coefficient1>1e-6)
                    {
                          //               printf("corner:s1.id=%d,s1.type=%d, s1  zero,s2 not zero,v2.x=%f,v2.y=%f,v2.z=%f,coe=%f\n",s1->ID,s1->type,v2.x,v2.y,v2.z,s2->length_coefficient[2]);
                        //xyz轴中s2轴动，s1轴不动
                        return 0.0;
                    }else
                    {
                       //   printf("corner:s1.id=%d,s1.type=%d, s1 zero,s2 zero\n",s1->ID,s1->type);
                    //xyz轴都不动，角动轴动
                        maxdiff = fabs(axis[3]);
                        for (i=4;i<9;i++)
                        {
...
static double sqGiveMaxVecWithFeedOverrate(SEGMENTQUEUE * sq, SEGMENT *s){
    if (s->gmodetype == 0 &&  sq->segpara_i[0] == 0) //g0是否受倍率控制
        return s->maxVec;
    else if  (s->gmodetype == 0)
         return s->maxVec*sq->feedOverrideFactor;
        else
...
//根据模式不同，给newseg设定不同的最大速度，最大加速度，最大法向加速度（处理拐点受限用）
    ...
    for ( i=3;i<9;i++)
    {
        kabc = max(kabc,fabs(newseg->length_coefficient[i]*sq->axis_rate[i]));
    }
    if((kabc>1) && (sq->abGoAlongXYZ != 0)) kabc = 1.0/kabc;
    else kabc = 1.0;
    if (G_number == 0) {
            newseg->maxVec = G_mode.maxV *kabc;
            newseg->gmodetype = 0;
    }
    else {
        newseg->maxVec = min(sq->feed,G_mode.maxV) *kabc;
        newseg->gmodetype = 1;
    }
...
*/
    //太短忽略掉
    if (fabs(circle.angle) <= 1e-6 || fabs(circle.angle)*circle.radius<0.002) {
        //only set ID of last appended motion
        sq->lastAppMotionID = ID;
        return 0;
    }
...
// 根据模式的不同，给newseg赋值最大速度
    ...
    newseg->maxVec = min(newseg->maxVec, G_mode.maxV)*kabc;
    newseg->givemaxVec = newseg->maxVec;
