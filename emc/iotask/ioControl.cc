...
struct iocontrol_str {
    ...
    hal_s32_t *z_flag;  /*1:z1 2:z2 3:z1+z2 ghf*/ 
...
int iocontrol_hal_init(void)
{
    ...
    // ghf z_flag
    retval = hal_pin_s32_newf(HAL_OUT, &(iocontrol_data->z_flag), comp_id, 
			      "iocontrol.%d.z_flag", n);
    if (retval < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR,
			"IOCONTROL: ERROR: iocontrol %d pin z_flag export failed with err=%i\n",
			n, retval);
	hal_exit(comp_id);
	return -1;
    }
...
int read_hal_inputs(void)
{
    ...
    if ( *(iocontrol_data->emc_enable_in)==0) //check for estop from HW,这个连到了net EstopSig [IO]E_STOP iocontrol.0.emc-enable-in 
