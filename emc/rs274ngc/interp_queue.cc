...
int Interp::move_endpoint_and_flush(setup_pointer settings, double x, double y) {
    ...
    for(unsigned int i = 0; i<qc().size(); i++) {
        ...
        case QSTRAIGHT_FEED: 
            ...
            //printf("moving endpoint of feed old dir %f new dir %f dot %f endpoint_valid %d x1:%.3f x2:%.3f y1:%.3f y2:%.3f\n", atan2(y1,x1), atan2(y2,x2), dot, endpoint_valid,x1,x2,y1,y2);
