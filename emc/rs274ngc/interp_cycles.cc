                              ...
                              CANON_SPEED_FEED_MODE mode)        //!< the speed-feed mode at outset      
{
  ...
  cycle_feed(block, plane, x, y, block->r_number);
  double k = -(_setup.speed*360)/_setup.feed_rate;
  if(_setup.motion_to_be == G_74)
      k *= -1;
  if(_setup.g84_location == 0) {        //G84开始攻牙
    _setup.g84_location = 1;
    STOP_SPINDLE_TURNING();
    WAIT(0, DIGITAL_INPUT, 3, 20);
    SET_AUX_OUTPUT_BIT(round_to_int(10));
    WAIT(10, DIGITAL_INPUT, 3, 20);
    CLEAR_AUX_OUTPUT_BIT(round_to_int(10));
    SET_AUX_OUTPUT_BIT(round_to_int(11)); //切换到位置模式
    SET_MULTI_DEF(84,1,0);        //不受倍率控制
     if(block->q_flag && block->q_number != 0) {            //只进一次执行该参数
         _setup.g84_needcycle = 1;
         _setup.g84_cycleqval = block->q_number;
     } else {
         _setup.g84_needcycle = 0;
         _setup.g84_cycleqval = 0;
     }
     if(block->p_flag && block->p_number != 0) {
         _setup.g84_cyclepval = block->p_number;
     } else {
         _setup.g84_cyclepval = 0;
     }

  }
  printf("now I get k:%.3f  s:%.3f  f:%.3f  bottom_z:%.3f  clearz:%.3f  cycle:%d  eflag:%d\n",k,_setup.feed_rate,_setup.speed,bottom_z,clear_z,_setup.g84_needcycle,block->e_flag);

  START_SPEED_FEED_SYNCH(k,2);
  if(_setup.g84_needcycle > 0) {
      double nowz = clear_z;
      while(nowz > bottom_z) {
          nowz -= _setup.g84_cycleqval;
          if(nowz < bottom_z)
              nowz = bottom_z;
          cycle_feed(block, plane, x, y, nowz);
          if(_setup.g84_cyclepval > 0)
              DWELL(_setup.g84_cyclepval);
          if(block->e_flag) {
              cycle_feed(block, plane, x, y, nowz+block->e_number);
          } else {
...
//  START_SPINDLE_CLOCKWISE();
  ...
  START_SPEED_FEED_SYNCH(k,-1); //停止同步，不断使能
                              ...
                              CANON_DIRECTION direction) //!< direction spindle turning at outset
{
  ...
  SET_AUX_OUTPUT_BIT(round_to_int(10));
  WAIT(10, DIGITAL_INPUT, 3, 20);
  if(_setup.parameters[4150] == 0) {
      cycle_feed(block, plane, x+block->q_number, y, bottom_z);
      cycle_traverse(block, plane, x+block->q_number, y, clear_z);
      cycle_traverse(block, plane, x, y, clear_z);
  } else if(_setup.parameters[4150] == 1) {
      cycle_feed(block, plane, x-block->q_number, y, bottom_z);
      cycle_traverse(block, plane, x-block->q_number, y, clear_z);
      cycle_traverse(block, plane, x, y, clear_z);
  } else if(_setup.parameters[4150] == 2) {
      cycle_feed(block, plane, x, y+block->q_number, bottom_z);
      cycle_traverse(block, plane, x, y+block->q_number, clear_z);
      cycle_traverse(block, plane, x, y, clear_z);
  } else if(_setup.parameters[4150] == 3) {
      cycle_feed(block, plane, x, y-block->q_number, bottom_z);
      cycle_traverse(block, plane, x, y-block->q_number, clear_z);
      cycle_traverse(block, plane, x, y, clear_z);
  }
                         ...
                         setup_pointer settings)        //!< pointer to machine settings                   
{
  ...
  if(!block->e_flag) {
      if (settings->motion_mode == motion && settings->cycle_e != -1) {
          block->e_flag = true;
          block->e_number = settings->cycle_e;
      }
  }


  if(motion == G_86) {          //fanuc格式支持 k是循环次数
      if(block->k_flag) {
          block->l_flag = block->k_flag;
          block->l_number = block->k_number;
      }
  }
  if(!block->l_flag && settings->cycle_l != 0 && motion == G_86) {
      block->l_number = settings->cycle_l;
  }
  ...
  settings->cycle_e = block->e_number;
                            ...
                            setup_pointer settings)     //!< pointer to machine settings                   
{
  ...
  if (old_cc < r) {
    ...
    old_cc = r;
    //此处需要更新一下current_z
    settings->current_z = r;
    current_cc = r;
  }  else {
      STRAIGHT_TRAVERSE(block->line_number, settings->current_x, settings->current_y, old_cc,
                        settings->AA_current, settings->BB_current, settings->CC_current,
                        settings->u_current, settings->v_current, settings->w_current);
      STRAIGHT_TRAVERSE(block->line_number, aa, bb, old_cc,
                        settings->AA_current, settings->BB_current, settings->CC_current,
                        settings->u_current, settings->v_current, settings->w_current);
...
//        NCE_DWELL_TIME_P_WORD_MISSING_WITH_G86);
    ...
    block->p_number =
      block->p_number == -1.0 ? settings->cycle_p : block->p_number;
    CHKS((block->q_number < 0 && block->q_flag), "Q值不能为负数");
