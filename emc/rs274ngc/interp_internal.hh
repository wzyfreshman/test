...
#define TOLERANCE_EQUAL 0.01 /* two numbers compare EQ if the
...
//hong  the minLength of the linearmove
...
typedef struct setup_struct
{
  ...
  double dynamicradius;     // radius of the dynamic compensation  add by hong 2015-04-01
  ...
  double cycle_e;               //e-val for canned cycles
  ...
  double feed_z_default; //feed_choice_mode 1 feed of  the z axis
  ...
  double rotation_offsetx; // rotation x offset of coordinate system add by hong on  20150831
  double rotation_offsety; // rotation y offset of coordinate system
  ...
  int runfromline;              //hong-2014/11/17 use for fast smartrun
  int only_reading_flag;        //hong-2014/04/14 use for the program verify
  double get_read_end_x;        //hong-2014/04/15 use for the program verify
  double get_read_end_y;        //hong-2014/04/15 use for the program verify
  double get_read_end_z;        //hong-2014/04/15 use for the program verify
  int motion_to_be;

  double g84_cyclepval;
  double g84_cycleqval;
  int g84_needcycle;            //攻牙循环下刀
  int g84_location;             //攻牙定位
  bool m99_flag;              //hong-2014/04/15 it will set true when exec the m99 block
  bool changez_flag;          //hong-2014/04/26 use for change z

  int z_mode;       //0 means linkage,1 means rotation
  int s_mode;       //hong-2014/05/30 use for enable or disable the s_number in the block
  int axis_mode;    //hong-2014/06/04 use for distinguish the number of axis
  int toolexist[CANON_POCKETS_MAX];
  bool zdiff_flag;
  double doublezdiff;
  double maxzdiff;  //zdiff cannot more than this value

  double doublezXdistance;
  double doublezYdistance;
  double doublezZdistance;

  double currentXdistance;
  double currentYdistance;
  double currentZdistance;

  bool m91_flag;    //use for m91 stop
  bool m70_flag;
  bool changcoorsys;
  int breakflag;    // code break flag


  char runafterset_text[LINELEN];   // linetext downcased, white space gone
  int runafterset;

  int startmove;
  int usegl;        //图形刷新标志位

  int points_in_circle;             //判断是否插入足够三个点
  EmcPose circlepoint[3];       //G2.1三点定圆弧
}
setup;
