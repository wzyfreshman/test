    ...
    if (block->g_modes[0] == G_53) {      /* distance mode is absolute in this case */
        ...
        if(block->x_flag) {
            *px = block->x_number - s->origin_offset_x - s->axis_offset_x - s->tool_offset.tran.x;
            if(_setup.parameters[4082] > 0)
                *px += s->currentXdistance;
        } else {
            *px = cx;
        }
        ...
        //printf("now get the currentz is %.3f %.3f\n",s->current_z,*pz);
...
int Interp::find_ends_2(block_pointer block,       //!< pointer to a block of RS274/NGC instructions
                      setup_pointer s,    //!< pointer to machine settings
                      double *px,        //!< pointer to end_x
                      double *py,        //!< pointer to end_y
                      double *pz,        //!< pointer to end_z
                      ...
                      double *u_p, double *v_p, double *w_p)
{
    ...
    if (block->g_modes[0] == G_53) {      /* distance mode is absolute in this case */
        ...
        *w_p = (block->w_flag) ? block->w_number : s->w_current;
        ...
        // both cutter comp planes affect X ...
        *px = (comp && middle) ? s->program_x: s->current_x - block->x_number;
        if(block->x_flag) *px += block->x_number;

        // but only XY affects Y ...
        *py = (comp && middle && s->plane == CANON_PLANE_XY) ? s->program_y: s->current_y - block->y_number;
        if(block->y_flag) *py += block->y_number;
        ...
        // and only XZ affects Z.
        *pz = (comp && middle && s->plane == CANON_PLANE_XZ) ? s->program_z: s->current_z - block->z_number;
        if(block->z_flag) *pz += block->z_number;
        ...
        *w_p = s->w_current;
        ...
        //printf("now get the zflag is %.3f  %.3f  %.3f %.3f\n",s->axis_offset_z,s->origin_offset_z,s->tool_offset.tran.z,*pz);
        *px += s->axis_offset_x;
        *py += s->axis_offset_y;
        *pz += s->axis_offset_z;
        *AA_p += s->AA_axis_offset;
        *BB_p += s->BB_axis_offset;
        *CC_p += s->CC_axis_offset;
        *u_p += s->u_axis_offset;
        *v_p += s->v_axis_offset;
        *w_p += s->w_axis_offset;
        ...
        *px += s->origin_offset_x  + s->tool_offset.tran.x;
        *py += s->origin_offset_y  + s->tool_offset.tran.y;
        *pz += s->origin_offset_z  + s->tool_offset.tran.z;
        *AA_p += s->AA_origin_offset + s->tool_offset.a;
        *BB_p += s->BB_origin_offset + s->tool_offset.b;
        *CC_p += s->CC_origin_offset + s->tool_offset.c;
        *u_p += s->u_origin_offset + s->tool_offset.u;
        *v_p += s->v_origin_offset + s->tool_offset.v;
        *w_p += s->w_origin_offset + s->tool_offset.w;
...
int Interp::find_tool_pocket(setup_pointer settings, int toolno, int *pocket)
{
    ...
    *pocket = -1;
    ...
    SET_SINGLE_OFFSET_TOOL(*pocket);  //add by hong
