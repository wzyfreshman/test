                         ...
                         setup_pointer settings)        //!< pointer to machine settings      
{
 ...
 //         NCE_CANNOT_USE_TWO_G_CODES_THAT_BOTH_USE_AXIS_VALUES);
      CHKS(((!axis_flag && !polar_flag) && 
            mode1 != G_0 && mode1 != G_1 && 
            mode1 != G_2 && mode1 != G_3 && mode1 != G_5_2  && mode1 != G_2_2),
...
//      printf("now I in here nog G pos %d  %d \n",block->motion_to_be, settings->motion_mode);
    CHKS((!settings->only_reading_flag) && ((settings->motion_mode == -1)
         || (settings->motion_mode == G_80)) && (block->g_modes[8] != G_43_1) &&
            block->g_modes[0] != G_68 && block->g_modes[0] != G_69,
        NCE_CANNOT_USE_AXIS_VALUES_WITHOUT_A_G_CODE_THAT_USES_THEM);

    if (block->g_modes[8] != G_43_1 && block->g_modes[0] != G_68 && block->g_modes[0] != G_69 ) {
       block->motion_to_be = settings->motion_mode;
    }
...
int Interp::parse_line(char *line,       //!< array holding a line of RS274 code  
                      ...
                      setup_pointer settings)   //!< pointer to machine settings         
{
  CHP(init_block(block));  //init all the block mode to -1
  ...
  settings->motion_to_be = block->motion_to_be;     //add by hong

  SET_G0_MODE(block->g_modes[0],block->g_modes[12]);
  return INTERP_OK;
}
