                        ...
                        double offset2)
{
  ...
  if (block->r_flag) { //caculate the center_x,center_y
  ...
  // caculate the feed_rate of the circle
  ...
  double length = find_arc_length(*current1, *current2, *current3, center1, center2,
                                  turn, end1, end2, end3);
                              ...
                              double u_end, double v_end, double w_end) //!< uvw at end of arc
{
    ...
    //printf("now find center: cx:%f cy:%f centerx:%f centery:%f endx:%f endy:%f blen:%f ABang:%f Aang:%f\n",cx,cy,center_x,center_y,end_x,end_y,b_len,AB_ang,A_ang);
    ...
    //printf("now I get the arc info: centerx:%f centery:%f clen:%f\n",center_x,center_y,c_len);
    ...
    double length = find_arc_length(cx, cy, cz, center_x, center_y,
                                    turn, end_x, end_y, end_z);
                              ...
                              double u, double v, double w) //!< uvw at end of arc
{
    ...
    //printf("now  compensation arc rad:%.3f centerx:%f centery:%f endx:%f endy:%f theta:%f delta:%f alpha:%f beta:%f gamma:%f side:%d opx:%f opy:%f cx:%f cy:%f  newendx:%f newendy:%f\n",tool_radius,centerx,centery,end_x,end_y,theta,delta,alpha,beta,gamma,side,opx,opy,cx,cy,new_end_x,new_end_y);
//beta角为前一段圆弧与后一段圆弧交点处两条半径的夹角
    if (beta < -small || 
        beta > M_PIl + small ||
           ...
           double l;
            if TOOL_INSIDE_ARC(side, turn) {
                ...
                toward_nominal = cy + tool_radius;
                 l = toward_nominal / dist_from_center;
            ...
            //printf("now I in get cy:%f towardnominal:%f discenter:%f anglefromcenter:%f l:%f midx:%f midy:%f\n",cy,toward_nominal,dist_from_center,angle_from_center,l,midx,midy);
            ...
            a = (SQ(oldrad) + SQ(arc_cc) - SQ(newrad)) / (2 * oldrad * arc_cc);  //余弦定理求夹角
    ...
    double length = find_arc_length(cx, cy, cz, centerx, centery,
                                    turn, new_end_x, new_end_y, end_z);
                                     ...
                                     setup_pointer settings)    //!< pointer to machine settings                  
{
  ...
  if(0 == _setup.parameters[4061] && g_code != 540) {
        return INTERP_OK;
  }
  switch (g_code) {
  case 540:
...
/*
  if(1 == _setup.z_mode && 4 == _setup.axis_mode) {
      if(8 == origin && settings->origin_index != 8) {
        parameters[5361] = parameters[5341] - _setup.doublezXdistance;
        parameters[5362] = parameters[5342] - _setup.doublezYdistance;
      }
      if(7 == origin && settings->origin_index != 7) {
        parameters[5341] = parameters[5361] + _setup.doublezXdistance;
        parameters[5342] = parameters[5362] + _setup.doublezYdistance;
      }
  }
*/

  //hong:in Z1+Z2,this function is useless,because after the zchange,the current coor
  //will update in synch() function
  ...
  //settings->rotation_xy = parameters[5210 + (origin * 20)];    //blocked by hong
  ...
  SET_XY_ROTATION(settings->rotation_xy,settings->rotation_offsetx,settings->rotation_offsety);
  printf("now I interp the changing coorsystem\n");
  _setup.changcoorsys = true;


  //刀库坐标切换
  if(_setup.parameters[4290] == 1) {
      double zoffset = _setup.parameters[4800+(int)_setup.parameters[5220]] + _setup.parameters[4303+((int)_setup.parameters[4288]*5)];
      double aoffset = _setup.parameters[4810+(int)_setup.parameters[5220]] + _setup.parameters[4703+((int)_setup.parameters[4289]*5)];

      char temp[100];
      sprintf(_setup.runafterset_text,"G90");
      if(_setup.block1.x_flag) {
          sprintf(temp,"X%.3f",_setup.block1.x_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.y_flag) {
          sprintf(temp,"Y%.3f",_setup.block1.y_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.z_flag) {
          sprintf(temp,"Z%.3f",_setup.block1.z_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.a_flag) {
          sprintf(temp,"A%.3f",_setup.block1.a_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.b_flag) {
          sprintf(temp,"B%.3f",_setup.block1.b_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.c_flag) {
          sprintf(temp,"C%.3f",_setup.block1.c_number);
          strcat(_setup.runafterset_text,temp);
      }
      if(_setup.block1.f_flag) {
          sprintf(temp,"F%.3f",_setup.block1.f_number);
          strcat(_setup.runafterset_text,temp);
      } else {
          sprintf(temp,"G0");
          strcat(_setup.runafterset_text,temp);
...
//      printf("get cmd text = %s %d %.3f %s\n",_setup.blocktext,_setup.block1.x_flag,_setup.block1.x_number,_setup.runafterset_text);
      char cmd[100];
      sprintf(cmd,"G10L2P[#5220]Z%.3fA%.3f",zoffset,aoffset);
      execute(cmd);
      _setup.runafterset = 1;
  }
...
int Interp::convert_cutter_compensation_off(setup_pointer settings)      //!< pointer to machine settings
{
  ...
  CHANGE_TOOL_NUMBER(settings->selected_pocket,0,0);         //add by hong 2014-06-30
  return INTERP_OK;
}
...
int Interp::convert_cutter_compensation_on(int side,     //!< side of path cutter is on (LEFT or RIGHT)
                                          ...
                                          setup_pointer settings)       //!< pointer to machine settings              
{
//    if(_setup.parameters[4127] > 0)
//         return INTERP_OK;

  double radius,realradius = 0.0;
  int pocket_number, orientation;
  int tool = settings->selected_pocket;
  ...
  if(block->g_modes[7] == G_41_1 || block->g_modes[7] == G_42_1) {
      ...
      CHKS((settings->plane != CANON_PLANE_XZ && orientation != 0 && orientation != 9),
           _("G%d with lathe tool, but plane is not G18"), block->g_modes[7]/10);
  }
  realradius = radius;
  ...
  START_CUTTER_RADIUS_COMPENSATION(side);
  CHANGE_TOOL_NUMBER(tool,side,realradius);         //add by hong 2014-06-30
  return INTERP_OK;
}
/****************************************************************************/
int Interp::convert_g53(block_pointer block, setup_pointer settings)
{
    // forbid by hong ,to ensure when execute g53,then it turn to g90 automatic
...
int Interp::convert_rotation(block_pointer block, setup_pointer settings) {
    if(block->g_modes[0] == G_69) {
        settings->rotation_xy = 0;
        settings->rotation_offsetx = 0;
        settings->rotation_offsety = 0;
        SET_XY_ROTATION(settings->rotation_xy,settings->rotation_offsetx,settings->rotation_offsety);
    } else if (block->g_modes[0] == G_68) {
        if(block->r_flag) {
            settings->rotation_xy = block->r_number;
        } else
            settings->rotation_xy = 0;

        if(block->x_flag) {
            settings->rotation_offsetx = block->x_number;
        } else
            settings->rotation_offsetx = 0;

        if(block->y_flag) {
            settings->rotation_offsety = block->y_number;
        } else
            settings->rotation_offsety = 0;
                             ...
                             setup_pointer settings)    //!< pointer to machine settings             
{
    ...
    if(0 == settings->feed_choice_mode || _setup.parameters[4060] > 0)
      enqueue_SET_FEED_RATE(block->f_number);
    else {
        enqueue_SET_FEED_RATE(settings->feed_default);
    }
  return INTERP_OK;
}
                     ...
                     setup_pointer settings)    //!< pointer to machine settings                 
{
  ...
  } else if (block->m_modes[5] == 70) {
      if(block->p_flag){
         _setup.m70_flag = true;
         SET_MULTI_DEF(block->p_number,block->l_number,block->q_number);
      }
  } else if (block->m_modes[5] == 71) {
      CHKS((!block->p_flag), _("No valid P word with M71"));
      SET_EXTERN_OUTPUT_BIT(round_to_int(block->p_number),1);
  } else if (block->m_modes[5] == 72) {
      CHKS((!block->p_flag), _("No valid P word with M72"));
      SET_EXTERN_OUTPUT_BIT(round_to_int(block->p_number),0);
  } else if (block->m_modes[5] == 80) {
      CHKS((settings->cutter_comp_side > 0), _("Cannot Break the code interping while compensation is on"));
      _setup.breakflag = 1;
      SET_SYNCH_CMD();
  } else if (block->m_modes[5] == 91) {
      _setup.m91_flag = true;
      char msg[256];
      if (block->comment[0] != 0) {
          strcpy(msg,block->comment);
          convert_param_comment(block->comment, msg,256);
      } else
          memset(msg,0,256);
      CHKS(!block->p_flag,_("Must specify both P with m91"));
      SET_WAIT_SIGNAL(block->p_number,msg);
  }
  ...
  if (block->m_modes[6] != -1) {
    ...
    CHANGE_TOOL_NUMBER(round_to_int(block->q_number),0,0);
    }

    // when we have M90 we change the Z axis

    if(block->m_modes[6] == 90) {
        int qflag = block->q_flag;
        int pnumber = (int)block->p_number;

//            CHKS((fabs(qnumber) - _setup.maxzdiff > 0), (_("zdiff is larger than the specify maxzdiff,please reset it")));

        double axisoffset[9] = {0};
        if(qflag > 0) {
            block->x_flag?axisoffset[0] = block->x_number:axisoffset[0] = 0;
            block->y_flag?axisoffset[1] = block->y_number:axisoffset[1] = 0;
            block->z_flag?axisoffset[2] = block->z_number:axisoffset[2] = 0;
            block->a_flag?axisoffset[3] = block->a_number:axisoffset[3] = 0;
            block->b_flag?axisoffset[4] = block->b_number:axisoffset[4] = 0;
            block->c_flag?axisoffset[5] = block->c_number:axisoffset[5] = 0;
            block->u_flag?axisoffset[6] = block->u_number:axisoffset[6] = 0;
            block->v_flag?axisoffset[7] = block->v_number:axisoffset[7] = 0;
            block->w_flag?axisoffset[8] = block->w_number:axisoffset[8] = 0;
        } else {
            axisoffset[0] = settings->origin_offset_x;
            axisoffset[1] = settings->origin_offset_y;
            axisoffset[2] = settings->origin_offset_z;
            axisoffset[3] = settings->AA_origin_offset;
            axisoffset[4] = settings->BB_origin_offset;
            axisoffset[5] = settings->CC_origin_offset;
            axisoffset[6] = settings->u_origin_offset;
            axisoffset[7] = settings->v_origin_offset;
            axisoffset[8] = settings->w_origin_offset;
        }
        if((int)block->p_flag == 0)
            pnumber = (int)_setup.parameters[4111];
        SET_DOUBLEZ_CHANGE(pnumber,qflag,&axisoffset[0]);

        _setup.changez_flag = true;
    }
  ...
  if (block->m_modes[7] == 3) {
      ...
      if(is_a_cycle(block->motion_to_be)) {
          printf("now In cycle ignore spindle\n");
      } else {

          CLEAR_AUX_OUTPUT_BIT(round_to_int(11)); //切换回速度模式
          enqueue_START_SPINDLE_CLOCKWISE();
          if (block->p_number != 0) {
              SET_MULTI_DEF(98,(int)block->p_number);
          }
      ...
      if(_setup.parameters[4128] == 0) {
          if (block->p_number== 1 || block->p_number== 2 ) {
              SET_MULTI_DEF(97,(int)block->p_number);
          } else {
  ...
  if (block->m_modes[8] == 7) {
      ...
      SET_EXTERN_OUTPUT_BIT(15,0);
  }

  if (block->m_modes[8] == 10) {
      LUBE_ON();
      settings->mist = true;
  } else if (block->m_modes[8] == 11) {
      LUBE_OFF();
      settings->flood = true;
  }
...
*/
  ...
  if(block->m_modes[11] != -1) {

      if(_setup.usegl > 0)
          return INTERP_OK;

        if(block->p_flag) {
            _setup.parameters[4058] = block->p_number;
        }
        if(block->q_flag) {
            _setup.parameters[4059] = block->q_number;
        }

        char mfilename[LINELEN];
        int index = block->m_modes[11];
        sprintf(mfilename,"%sm%d.ngc",_setup.subngcpath,index);
        printf("now I get mfilename is %s\n",mfilename);
        FILE *mfile;
        ...
        sprintf(mfilename,"O<M%d> call",index);
        execute(mfilename);
//        if (EMC_DEBUG & EMC_DEBUG_INTERP)
//            printf("subroutines is %s\n",_setup.subngcpath);
//        char mfilename[LINELEN];
//        int index = block->m_modes[11];
//        sprintf(mfilename,"%sM%d",_setup.subngcpath,index);
//        FILE *mfile;
//        if((mfile = fopen(mfilename,"r")) == NULL) {
//          CHKS(1, NCE_UNKNOWN_M_CODE_USED);
//          return INTERP_ERROR;
...
//            char cmd[LINELEN];
//            while(!feof(mfile)) {
//              if (fgets(cmd,sizeof(cmd),mfile) != NULL) {
//                  if (EMC_DEBUG & EMC_DEBUG_INTERP)
//                    printf("now get the m command %s\n",cmd);
//                  execute(cmd);
//              }
//            }
                           ...
                           setup_pointer settings)      //!< pointer to machine settings                 
{
  ...
  if (code == G_10) {
    ...
    CHP(convert_nurbs(code, block, settings));
      ...
      CHP(convert_g53(block,settings));
  } else if (code == G_68 || code == G_69) {
      CHP(convert_rotation(block,settings));
  } else if(code == G_4);  /* handled elsewhere */
  else
                          ...
                          setup_pointer settings)       //!< pointer to machine settings              
{
...
#endif
    if(_setup.g84_location > 0) {
     _setup.g84_location = 0;
     _setup.g84_needcycle = 0;
     _setup.g84_cyclepval = 0;
     _setup.g84_cycleqval = 0;

     STOP_SPEED_FEED_SYNCH();       //停止主轴同步
     SET_MULTI_DEF(84,0,0);      //解除不受倍率控制
     CLEAR_AUX_OUTPUT_BIT(round_to_int(11)); //切换回速度模式


     if(settings->spindle_turning == CANON_CLOCKWISE)
        START_SPINDLE_CLOCKWISE();
     else if(settings->spindle_turning == CANON_COUNTERCLOCKWISE)
  ...
  }  else if (motion == G_2_1) {
      CHP(convert_point2circle(block, settings));
    }  else if (motion == G_2_2) {
      CHP(convert_three2circle(block, settings));
    } else {
                          ...
                          setup_pointer settings)        //!< pointer to machine settings             
{
  ...
  /* send the probe position as the machine coor */
  CHP(find_ends(block, settings, &end_x, &end_y, &end_z,
                &AA_end, &BB_end, &CC_end,
                &u_end, &v_end, &w_end));
  CHKS(((!(probe_type & 1)) &&
  ...
  printf("now get the probeendz is %.3f\n",end_z);
  TURN_PROBE_ON();
                         ...
                         setup_pointer settings)        //!< pointer to machine settings                 
{
  ...
  if (p_int == settings->origin_index) {        /* system is currently used */
    ...
    printf("now setting offsetz %.3f  current z is %.3f \n",settings->origin_offset_z,settings->current_z);
    ...
    AxisOffset[0] = settings->origin_offset_x ;
    AxisOffset[1] = settings->origin_offset_x ;
    AxisOffset[2] = settings->origin_offset_x ;
    AxisOffset[3] = a;
    AxisOffset[4] = b;
    AxisOffset[5] = c;
    AxisOffset[6] = u;
    ...
    SET_G5X_OFFSET(p_int, x, y, settings->origin_offset_z, a, b, c, u, v, w);
    SET_DOUBLEZ_CHANGE(-1,0,&AxisOffset[0]);

    rotate(&settings->current_x, &settings->current_y, - r);

    //settings->rotation_xy = r;    blocked by hong on 2015-08-31
    //SET_XY_ROTATION(settings->rotation_xy,settings->rotation_offsetx,settings->rotation_offsety);


    //add by hong ,after set coors,set the var in varfile
    char file_name[256];
  ...
  if(_setup.parameters[4290] == 1) {
           _setup.parameters[4800+p_int] = \
              -_setup.parameters[4303+((int)_setup.parameters[4288]*5)]+_setup.parameters[5203+p_int*20];
      _setup.parameters[4810+p_int] = \
              -_setup.parameters[4703+((int)_setup.parameters[4289]*5)]+_setup.parameters[5204+p_int*20];
      printf("now I get in here set :%f %f %f   end:%f  %d\n",_setup.parameters[4290] ,_setup.parameters[4303+((int)_setup.parameters[4288]*5)],_setup.parameters[5203+p_int*20],_setup.parameters[4800+p_int],(int)_setup.parameters[4288]);
  }

  _setup.changcoorsys = true;

  if(_setup.runafterset > 0) {
    execute(_setup.runafterset_text);
...
//hong-2014/04/15  add the M99 convert
...
*/
    ...
    if (_setup.file_pointer) {      // modify by hong: forbid the character '%'
    ...
    return INTERP_EXIT;
  } else if(block->m_modes[4] == 99){
    if(_setup.only_reading_flag > 0)
	return INTERP_OK;
      fseek(_setup.file_pointer,0,SEEK_SET);
      unwind_call(INTERP_EXIT, __FILE__,__LINE__); //hong use for reset
      _setup.m99_flag = true;
      _setup.parameters[4040] = 1;
    SET_M99_EXECUTE();
    return INTERP_OK;
  } else
                            ...
                            setup_pointer settings)     //!< pointer to machine settings             
{
  ...
  if(((move == G_0) || (move == G_1))  && !block->x_flag && !block->y_flag && !block->z_flag &&
          !block->a_flag && !block->b_flag && !block->c_flag ) {
      return INTERP_OK;
  }
 ...
 // printf("########current x=%.3f y=%.3f z=%.3f\n",settings->current_x,settings->current_y,settings->current_z);
  CHP(find_ends(block, settings, &end_x, &end_y, &end_z,
                &AA_end, &BB_end, &CC_end, &u_end, &v_end, &w_end));
//  printf("now find endx=%.3f  end_y=%.3f end_z=%.3f a:%.3f b:%.3f c:%.3f\n",end_x,end_y,end_z,AA_end,BB_end,CC_end);


  if(1 < settings->feed_choice_mode) {
      double dx = end_x - settings->current_x;
      double dy = end_y - settings->current_y;
      double dz = end_z - settings->current_z;
      static bool changezflag = false;
      if(2 == settings->feed_choice_mode) {
          if(dz < 0 && fabs(dx) == 0 && fabs(dy) == 0) {
                enqueue_SET_FEED_RATE(settings->feed_z_default);
                changezflag = true;
          } else if (changezflag && (fabs(dx) > 0 || fabs(dy) > 0)) {
                enqueue_SET_FEED_RATE(settings->feed_default);
                changezflag = false;
          }
      } else if (3 == settings->feed_choice_mode) {
          if(dz < 0) {
                enqueue_SET_FEED_RATE(settings->feed_z_default);
                changezflag = true;
          } else if (changezflag && (fabs(dx) > 0 || fabs(dy) > 0)) {
  ...
  findRealLength(end_x, end_y, end_z,
                 AA_end, BB_end, CC_end,
                 u_end, v_end, w_end,
                 length, settings);

//  if(_setup.parameters[4083] > 0) {
//      if(length < _setup.parameters[4083])
//          return INTERP_OK;
//  }

//  printf("id=%d cmd:%s lenght=%.3f now find endx=%.3f end_y=%.3f end_z=%.3f A:%.3f C:%.3f startmove:%d\n",settings->sequence_number, settings->blocktext,length,end_x,end_y,end_z,AA_end,CC_end,_setup.startmove);

     if(_setup.startmove == 0) {
         double dx = end_x - settings->current_x;
         double dy = end_y - settings->current_y;
         double dz = end_z - settings->current_z;
         if(fabs(dx) > 0 && fabs(dy) > 0  && fabs(dz) > 0 && _setup.parameters[4129] > 0) {

             if(move == G_0) {
                     STRAIGHT_TRAVERSE(block->line_number, end_x, end_y, settings->current_z,
                                   AA_end, BB_end, CC_end,
                                   u_end, v_end, w_end);
                     STRAIGHT_TRAVERSE(block->line_number, end_x, end_y, end_z,
                                   AA_end, BB_end, CC_end,
                 ...
                 STRAIGHT_FEED(block->line_number, end_x, end_y, settings->current_z,
                               AA_end, BB_end, CC_end,
                               u_end, v_end, w_end);
     ...
     if(_setup.parameters[4125] > 0 && move == G_0) {
         double dx = end_x - settings->current_x;
         double dy = end_y - settings->current_y;
         double dz = end_z - settings->current_z;
         if((fabs(dx) > 0 || fabs(dy)) > 0 && fabs(dz) > 0) {
             if(dz > 0) {
                 STRAIGHT_TRAVERSE(block->line_number, settings->current_x, settings->current_y, end_z,
                               AA_end, BB_end, CC_end,
                               u_end, v_end, w_end);
                 STRAIGHT_TRAVERSE(block->line_number, end_x, end_y, end_z,
                               AA_end, BB_end, CC_end,
                                   ...
                                   double u_end, double v_end, double w_end)
{
    double alpha;
    double distance;
    double radius; /* always will be positive */
    double end_x, end_y;
    ...
    //printf("now  compensation radius is %.3f\n",radius);

    comp_get_current(settings, &cx, &cy, &cz);
    distance = hypot((px - cx), (py - cy));

    CHKS(((side != LEFT) && (side != RIGHT)), NCE_BUG_SIDE_NOT_RIGHT_OR_LEFT);
    /** blocked by hong on 12-06,2015,and I think it is still ok in this condition**/
                                   ...
                                   double u_end, double v_end, double w_end)
{
    ...
    if ((py == opy) && (px == opx)) {     /* no XY motion */
        ...
        //printf("opx=%.3f,cx=%.3f,opy=%.3f,cy=%.3f,theta=%.3f,alpha=%.3f,beta=%.3f  mdix:%f  midy:%f\n"
        //       ,opx,cx,opy,cy,theta,alpha,beta,mid_x,mid_y);
        ...
        if (!concave && (beta > small)) {       /* ARC NEEDED */
            CHP(move_endpoint_and_flush(settings, cx, cy));
           // printf("now Insert ARC :midx:%f midy:%f cx:%f cy:%f opx:%f opy:%f gamma:%f\n",mid_x,mid_y,cx,cy,opx,opy,gamma);
            if(move == G_1) {
                enqueue_ARC_FEED(settings, block->line_number,
...
//                printf("now In line->line:midx:%f midy:%f cx:%f cy:%f retreat:%f theta:%f gamma:%f\n",mid_x,mid_y,cx,cy,retreat,theta,gamma);
                ...
                //printf("in here 4 x=%.3f  y=%.3f\n",mid_x,mid_y);
                CHP(move_endpoint_and_flush(settings, mid_x, mid_y));
            }
        } else {
            // no arc needed, also not concave (colinear lines or tangent arc->line)
            //printf("in here 2 x=%.3f  y=%.3f\n",cx,cy);
            dequeue_canons(settings);
            set_endpoint(cx, cy);
        }

        //if(fabs(fabs(alpha)-0)<0.001 || fabs(fabs(alpha)-M_PIl)<0.01 || fabs(fabs(alpha)-M_PI_2l)<0.01 || fabs(fabs(alpha)-4.711)<0.01)
        //end_x -= radius*(end_x-15>0?-1:1);
        //printf("in here 3 x=%.3f  y=%.3f\n",end_x,end_y);
...
{
    if(settings->selected_pocket > _setup.parameters[3811]) {
        ERS("换刀刀号超过最大刀具数量!");
    }
    _setup.parameters[4304] = _setup.parameters[4300];
    _setup.parameters[4300] = settings->selected_pocket;
    _setup.parameters[4303] = 0;
    CHANGE_TOOL(settings->selected_pocket);
    if(_setup.parameters[4127] == 0 &&
            _setup.usegl == 0) {                                //刷新模型不需要跑换刀程序
        settings->toolchange_flag = 1;
//        _setup.parameters[4294] = 1;    //开始换刀标志,改在task执行
        execute("O<toolchange> call");
    }

   if(_setup.z_mode > 0 && (1 == settings->selected_pocket%2 || 0 == settings->selected_pocket%2)) {
        //execute("g53g0z0");
        init_block(&(_setup.block1));  //if we execute cmd in the interplist ,then we need to init the block

        if(1 == settings->selected_pocket) {
            _setup.currentXdistance = 0;
            _setup.currentYdistance = 0;
            _setup.currentZdistance = 0;
            SET_ROTATION_DIS(0,0);
            SET_DOUBLEZ_CHANGE(0,0,0);
        } else if(2 == settings->selected_pocket) {
            _setup.currentXdistance = _setup.doublezXdistance;
            _setup.currentYdistance = _setup.doublezYdistance;
            _setup.currentZdistance = _setup.doublezZdistance;
            SET_ROTATION_DIS(_setup.doublezXdistance,_setup.doublezYdistance,_setup.doublezZdistance);
            SET_DOUBLEZ_CHANGE(1,0,0);
        }
        settings->toolchange_flag = 1;
...
//         return INTERP_OK;
  ...
  if (g_code == G_49) {
    ...
    if(_setup.parameters[4000] == 0 || _setup.parameters[4000] == 2)
    tool_offset.tran.z = USER_TO_PROGRAM_LEN(settings->tool_table[pocket_number].offset.tran.z);
    else if (_setup.parameters[4000] == 1)
        tool_offset.tran.z = USER_TO_PROGRAM_LEN(settings->tool_table[pocket_number].offset.a);
  ...
  } else if(g_code == G_43_4) {
      SET_MULTI_DEF(52,1,0);
    return INTERP_OK;
  } else {
  ...
  if(_setup.parameters[4000]  == 2 && _setup.parameters[4109]  > 0 && (
          (settings->tool_table[settings->selected_pocket].offset.tran.z != 0 ||
           settings->tool_table[settings->selected_pocket].offset.a != 0 ||
           settings->tool_table[settings->selected_pocket].offset.tran.z !=
           settings->tool_table[(int)_setup.parameters[4304]].offset.tran.z) ||
              settings->tool_table[settings->selected_pocket].offset.a !=
              settings->tool_table[(int)_setup.parameters[4304]].offset.a))
           {
      double diff = 0;
      char cmdbuf[256];
      diff = _setup.origin_offset_z - _setup.AA_origin_offset + tool_offset.tran.z - tool_offset.a;
      printf("[convert_tool_length_offset] get the zdiff is %f\n",diff);
    sprintf(cmdbuf,"M90P3Q%f",diff);
    execute(cmdbuf);
  }
...
//    if((_setup.axis_mode==4 && _setup.z_mode != 0 ) || _setup.axis_mode==3) {
    //忽略程序中的刀号
    if(_setup.parameters[4127] > 0)
         return INTERP_OK;
        ...
        _setup.parameters[3814] = pocket;
        printf("Now I select tool ========================= %d  m_mode:%d\n",pocket,block->m_modes[6]);
        if(block->m_modes[6] == -1) {
            execute("O<toolselect> call");
        }

    CHANGE_TOOL_NUMBER(block->t_number,0,0);
    return INTERP_OK;
}

int Interp::convert_point2circle(block_pointer block,setup_pointer settings) {

    CHKS((!block->p_flag), (("G2.1必须指定为第几个点")));
    int index  = (int)block->p_number-1;
    if(block->x_flag)
        settings->circlepoint[index].tran.x = block->x_number;
    else
        settings->circlepoint[index].tran.x = _setup.current_x;

    if(block->y_flag)
        settings->circlepoint[index].tran.y = block->y_number;
    else
        settings->circlepoint[index].tran.y = _setup.current_y;

    if(block->z_flag)
        settings->circlepoint[index].tran.z = block->z_number;
    else
        settings->circlepoint[index].tran.z = _setup.current_z;

    if(block->a_flag)
        settings->circlepoint[index].a = block->a_number;
    else
        settings->circlepoint[index].a = _setup.AA_current;

    if(block->b_flag)
        settings->circlepoint[index].b = block->b_number;
    else
        settings->circlepoint[index].b = _setup.BB_current;

    if(block->c_flag)
        settings->circlepoint[index].c = block->c_number;
    else
        settings->circlepoint[index].c = _setup.CC_current;

    if( (int)block->p_number == 3)
        _setup.points_in_circle = 1;

    settings->current_x = settings->circlepoint[index].tran.x;
    settings->current_y = settings->circlepoint[index].tran.y;
    settings->current_z = settings->circlepoint[index].tran.z;
    settings->AA_current = settings->circlepoint[index].a;
    settings->BB_current = settings->circlepoint[index].b;
    settings->CC_current = settings->circlepoint[index].c;

    return INTERP_OK;
}
int Interp::convert_three2circle(block_pointer block,setup_pointer settings) {
    if(_setup.points_in_circle == 0) {
        CHKS((!false), (("G2.2之行前必须先使用G2.1添加三个点")));
    }
    ...
    double P1[3],P2[3],P3[3],center[3],normal[3];
    P1[0] = settings->circlepoint[0].tran.x;
    P1[1] = settings->circlepoint[0].tran.y;
    P1[2] = settings->circlepoint[0].tran.z;

    P2[0] = settings->circlepoint[1].tran.x;
    P2[1] = settings->circlepoint[1].tran.y;
    P2[2] = settings->circlepoint[1].tran.z;

    P3[0] = settings->circlepoint[2].tran.x;
    P3[1] = settings->circlepoint[2].tran.y;
    P3[2] = settings->circlepoint[2].tran.z;

    double flag = 0;
    computerCircleWith3point(P1, P2,P3, &flag, center, normal);
    centerx = center[0];
    centery = center[1];
    centerz = center[2];
    ...
    a=settings->circlepoint[2].a;
    b=settings->circlepoint[2].b;
    c=settings->circlepoint[2].c;

    if(flag > 0) {
        STRAIGHT_FEED(block->line_number,  P1[0],P1[1],P1[2],settings->circlepoint[0].a,settings->circlepoint[0].b,settings->circlepoint[0].c, 0, 0, 0);
        THREE_2_CIRCLE(block->line_number, x,y,z,a,b,c,centerx,centery,centerz,normalx,normaly,normalz);
    } else {
        STRAIGHT_FEED(block->line_number, x, y, z, a, b, c, 0, 0, 0);
    }

    printf("G2.2 get Endpoint x:%.3f y:%.3f z:%.3f centerx:%.3f centery:%.3f centerz:%.3f normalx:%.3f normaly:%.3f normalyz:%.3f\n",\
           x,y,z,centerx,centery,centerz,normalx,normaly,normalz);
    settings->current_x = x;
    settings->current_y = y;
    settings->current_z = z;
    settings->AA_current = a;
