                         ...
                         setup_pointer settings)        //!< pointer to machine settings                 
{
  ...
  if (settings->m91_flag) {
    settings->m91_flag = false;
    return INTERP_EXECUTE_FINISH;
  }

  if (settings->m70_flag) {
      settings->m70_flag = false;
      return INTERP_EXECUTE_FINISH;
  }

  if(settings->changez_flag) {
      printf("now m91 finish\n");
      settings->changez_flag = false;
      return INTERP_EXECUTE_FINISH;
  }

  if(_setup.changcoorsys) {
      _setup.changcoorsys = false;
      return INTERP_EXECUTE_FINISH;
  }

  if( _setup.breakflag > 0) {
      _setup.breakflag = 0;
      return INTERP_EXECUTE_FINISH;
  }
