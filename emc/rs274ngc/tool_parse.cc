			 ...
			 int random_toolchanger)
{
    ...
    for (t = random_toolchanger? 0: 1; t < CANON_POCKETS_MAX; t++) {
        ...
        toolTable[pocket].arrayrow[0] = 0;
        toolTable[pocket].arrayrow[1] = 0;
        toolTable[pocket].arraycolumn[0] = 0;
        toolTable[pocket].arraycolumn[1] = 0;
        if(fms) fms[t] = 0;
    ...
    */
    ...
    while (!feof(fp)) {
        ...
        double arrayrow1,arrayrow2,arraycolumn1,arraycolumn2;
        EmcPose offset;  // tlo
        ...
        while (token != NULL) {
            ...
            case 'Q':
                ...
                break;
            case 'M':
                if (sscanf(&token[1], "%lf", &arrayrow1) != 1)
                    valid = 0;
                break;
            case 'N':
                if (sscanf(&token[1], "%lf", &arrayrow2) != 1)
                    valid = 0;
                break;
            case 'E':
                if (sscanf(&token[1], "%lf", &arraycolumn1) != 1)
                    valid = 0;
                break;
            case 'F':
                if (sscanf(&token[1], "%lf", &arraycolumn2) != 1)
                    valid = 0;
                break;
        ...
        if (valid) {
            ...
            toolTable[pocket].backangle = backangle;
            toolTable[pocket].orientation = orientation;
            toolTable[pocket].arrayrow[0] = arrayrow1/2.0;
            toolTable[pocket].arrayrow[1] = arrayrow2/2.0;
            toolTable[pocket].arraycolumn[0] = arraycolumn1/2.0;
            toolTable[pocket].arraycolumn[1] = arraycolumn2/2.0;
            //printf("pokey %d x %d %.3f tool 0= %.3f tool 1=%.3f  xxxx %f\n",pocket,toolTable[pocket].toolno,toolTable[pocket].arrayrow[1],toolTable[pocket].arraycolumn[0],toolTable[pocket].arraycolumn[1],arraycolumn2);
