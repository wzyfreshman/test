                      ...
                      double *parameters)   //!< array of system parameters 
{
  ...
  //hong 2014-05-04 ignore the '%' in the program execpt the head or end of the file
  if (line[counter] == '%')
      counter++;
                  ...
                  double *parameters)   //!< array of system parameters                   
{
  ...
  *counter = (*counter + 1);
  CHP(read_real_value(line, counter, &value_read, parameters));
  value = floor(value_read);
  CHKS((value < 0), NCE_NEGATIVE_M_CODE_USED);
  CHKS((value > 299), NCE_M_CODE_GREATER_THAN_199);
  mode = _ems[value];
...
#define CMP(txt) (strncmp(line+*counter, txt, strlen(txt)) == 0 && (*counter += strlen(txt)))
  ...
  if(block->o_type == O_sub)
  ...
  else
    {
      ...
      //forbid o_code error output by hong //2014-05-16
    }
                            ...
                            double *double_ptr) //!< pointer to double to be read                  
{
  ...
  //add by HONG ,solve the '.' problem in the program
  dot_flag = st.find_first_of('.',0);
  if(-1 != dot_flag){
      int temp = 0;
    do{
        temp = 0;
        temp = st.find_first_of('.',dot_flag+1);
        if(-1 != temp)
            st.erase(temp);
        dot_flag = temp;
      }while(-1 != temp);
    ...
    int *length)       //!< a pointer to an integer to be set
{
  ...
  if (command == NULL) {
    //fast location the smartrunline of the file
      static double xval=0,yval=0,zval=0,aval=0,bval=0,cval=0,fval=6000;
      int getoutflag = 0;
      static int getoutflag2 = 0,circlemode = 0,gcycle = 0,g43flag = 0,comptool = 1,nowtool = 1,dircomp = 0,dirtool = 1;
      static char lastline[LINELEN];
      double partemp[RS274NGC_MAX_PARAMETERS];
      static char gcyclecmd[LINELEN],gcyclecmd1[LINELEN];
      block blocktemp;
      while(_setup.runfromline > 0) {
        blocktemp.x_flag = false;blocktemp.x_number = 0;
        blocktemp.y_flag = false;blocktemp.y_number = 0;
        blocktemp.z_flag = false;blocktemp.z_number = 0;
        blocktemp.f_flag = false;blocktemp.f_number = 0;
        blocktemp.a_flag = false;blocktemp.a_number = 0;
        blocktemp.b_flag = false;blocktemp.b_number = 0;
        blocktemp.c_flag = false;blocktemp.c_number = 0;
        blocktemp.t_flag = false;blocktemp.t_number = 0;
        blocktemp.p_flag = false;blocktemp.p_number = -1;
        blocktemp.l_flag = false;blocktemp.l_number = -1;
        strcpy(lastline,raw_line);

        temp = fgets(raw_line, LINELEN, inport);
        for (n = 0; n < 16; n++)
          blocktemp.g_modes[n] = -1;
        for (n = 0; n < 12; n++) {
          blocktemp.m_modes[n] = -1;
        }
        close_and_downcase(raw_line);
        read_items(&blocktemp,raw_line,partemp);
        if(blocktemp.x_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30 ){
                xval = blocktemp.x_number - _setup.origin_offset_x-_setup.axis_offset_x;
            } else{
                xval = blocktemp.x_number;
            }
        }
        if(blocktemp.y_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30 ) {
                yval = blocktemp.y_number - _setup.origin_offset_y-_setup.axis_offset_y;
            } else
                yval = blocktemp.y_number;
        }
        if(blocktemp.z_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30  ) {
                zval = blocktemp.z_number - _setup.origin_offset_z-_setup.axis_offset_z;
            } else
                zval = blocktemp.z_number;
        }
        if(blocktemp.a_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30 ) {
                aval = blocktemp.a_number - _setup.AA_origin_offset-_setup.AA_axis_offset;
            } else
                aval = blocktemp.a_number;
        }
        if(blocktemp.b_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30  ) {
                bval = blocktemp.b_number - _setup.BB_origin_offset-_setup.BB_axis_offset;
            } else
                bval = blocktemp.b_number;
        }
        if(blocktemp.c_flag) {
            if(blocktemp.g_modes[0] == G_53 ||blocktemp.g_modes[0] == G_28  || blocktemp.g_modes[0] == G_30  ) {
                cval = blocktemp.c_number - _setup.CC_origin_offset-_setup.CC_axis_offset;
            } else
                cval = blocktemp.c_number;
        }
        if(blocktemp.t_flag) {
            nowtool = blocktemp.t_number;
            printf("now I get tool is ======================================  %d \n",nowtool);
        }
        if(blocktemp.f_flag) {
            fval = blocktemp.f_number;
        }
        if(blocktemp.g_modes[1] == G_2 || blocktemp.g_modes[1]  == G_3) {
            if(blocktemp.g_modes[1]  == G_2)
                circlemode = 1;
            else
                circlemode = 2;
//            printf("Now I in set circlemode \n");
        } else if(blocktemp.g_modes[1]  == G_0 || blocktemp.g_modes[1]  == G_1) {
            circlemode = 0;
//            printf("Now I in clear circlemode \n");
        } else if(blocktemp.g_modes[1]  == G_81 || blocktemp.g_modes[1]  == G_82 || blocktemp.g_modes[1]  == G_83 || blocktemp.g_modes[1]  == G_84 || blocktemp.g_modes[1]  == G_73) {
            gcycle = 1;
            memset(gcyclecmd,0,sizeof(gcyclecmd));
            memset(gcyclecmd1,0,sizeof(gcyclecmd1));

            strcpy(gcyclecmd1,lastline);
            strcpy(gcyclecmd,raw_line);
//            printf("Now I get cycle cmd:%s",gcyclecmd);
        } else if(blocktemp.g_modes[1]  == G_80) {
            gcycle = 0;
        }

        if(blocktemp.g_modes[8] == G_43) {
            g43flag = 1;
            comptool = (int)blocktemp.h_number;
        } else if(blocktemp.g_modes[8] == G_49) {
            g43flag = 0;
        }

        if(blocktemp.g_modes[7] == G_41 ||  blocktemp.g_modes[7] == G_42) {
            if(blocktemp.g_modes[7] == G_41)
                dircomp = 1;
            else
                dircomp = 2;
            dirtool = (int)blocktemp.d_number_float;
        } else if(blocktemp.g_modes[7] == G_40) {
            dircomp = 0;
        }
      ...
      if(1 == getoutflag) {
          sprintf(raw_line,"G53G0Z0");
          temp = raw_line;
          _setup.sequence_number--;
            getoutflag2 = 2;

      }  else if (getoutflag2 == 2 && _setup.call_level == 0) {

            sprintf(raw_line,"T%dM6",nowtool);
          _setup.sequence_number--;
          temp = raw_line;
            getoutflag2 = 3;
            printf("Now I in 2 raw_line:%s  %d\n",raw_line,nowtool);
      } else  if (getoutflag2 == 3 && _setup.call_level == 0) {

          sprintf(raw_line,"G53G0Z0");
          _setup.sequence_number--;
          temp = raw_line;
          getoutflag2 = 4;
          printf("Now I in 3 raw_line:%s\n",raw_line);
      } else  if (getoutflag2 == 4 && _setup.call_level == 0) {

            sprintf(raw_line,"G0X%.3fY%.3f",xval,yval);

          _setup.sequence_number--;
          temp = raw_line;
          getoutflag2 = 5;
          printf("Now I in 4 raw_line:%s\n",raw_line);
      } else  if (getoutflag2 == 5 && _setup.call_level == 0) {

          if(g43flag > 0) {
              sprintf(raw_line,"G43H%d",comptool);
          } else {
            sprintf(raw_line,"");
          }
          ...
          temp = raw_line;
          getoutflag2 = 6;
          printf("Now I in 5 raw_line:%s\n",raw_line);
      } else if(getoutflag2 == 6 && _setup.call_level == 0) {

          sprintf(raw_line,"G1Z%.3fF%.3f",zval,fval);
          _setup.sequence_number--;
          temp = raw_line;
          printf("Now I in 6 raw_line:%s\n",raw_line);
          getoutflag2 = 7;
      }else if(getoutflag2 == 7 && _setup.call_level == 0) {

          if(dircomp > 0) {
              if(dircomp == 1)
              sprintf(raw_line,"G41D%d",dirtool);
              else if(dircomp == 2)
              sprintf(raw_line,"G42D%d",dirtool);
          } else {
                sprintf(raw_line,"");
          }
          ...
          _setup.sequence_number--;
          temp = raw_line;
          printf("Now I in 7 raw_line:%s\n",raw_line);
          getoutflag2 = 8;
      } else  if (getoutflag2 == 8 && _setup.call_level == 0) {
          ...
          if(circlemode == 0) {
            sprintf(raw_line," ");
              ...
              if(strstr(raw_line,"G2") == NULL && strstr(raw_line,"g2") == NULL && strstr(raw_line,"G02") == NULL && strstr(raw_line,"g02") == NULL &&
                      strstr(raw_line,"G3") == NULL && strstr(raw_line,"g3") == NULL && strstr(raw_line,"G03") == NULL && strstr(raw_line,"g03") == NULL) {
                  if(circlemode == 1)
                      sprintf(tempdate,"G2%s",temp);
                  else if(circlemode == 2)
                      sprintf(tempdate,"G3%s",temp);
                  sprintf(raw_line,"%s",tempdate);
//                  raw_line = tempdate;
                  printf("Now I in 733 raw_line:%s temp:%s tempdate:%s\n",raw_line,temp,tempdate);
              } else {
    ...
    if (temp == NULL) {
      ...
      if(_setup.only_reading_flag > 0)  //hong-2014/04/14
      {                             //use for program verify
        return INTERP_EXIT;
      }
      ...
      if(_setup.skipping_to_sub)
      {
        ERS(_("EOF in file:%s seeking o-word: o<%s> from line: %d"),
          ...
          FINISH();
          unwind_call(INTERP_EXIT, __FILE__,__LINE__);      //hong use for sync the o_type cmd
