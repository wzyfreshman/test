...
static int loadAxis(int axis, EmcIniFile *axisIniFile)
{
    ...
    /*switch(axis){
    case 0:sprintf(axisStringhome, "IO_XHOME");break;
    case 1:sprintf(axisStringhome, "IO_YHOME");break;
    case 2:sprintf(axisStringhome, "IO_ZHOME");break;
    case 3:sprintf(axisStringhome, "IO_Z2HOME");break;
    default:break;
    ...
    try {
        ...
        axisIniFile->Find(&seclimit,"SECMIN_LIMIT",axisString);
        axisIniFile->Find(&posoffset,"SECLIMIT_OFFSET",axisString);
        ...
        axisIniFile->Find(&seclimit,"SECMAX_LIMIT",axisString);
        axisIniFile->Find(&posoffset,"SECLIMIT_OFFSET",axisString);
        ...
        ignore_limits = !ignore_limits;     //add by hong because the describe of this parameter is opposite
        sequence = -1;	                // default
        ...
        //ghf-2014/6/24
        pitchvalue = 0;
        axisIniFile->Find(&pitchvalue, "PITCHVALUE", axisString);

        //ghf-2014/11/28
        fric_delay_t=5;
        axisIniFile->Find(&fric_delay_t, "FRIC_DELAY_T", axisString);
        fric_lag_ts0=25;
        axisIniFile->Find(&fric_lag_ts0, "FRIC_LAG_TS0", axisString);
        fric_lag_ts1=25;
        axisIniFile->Find(&fric_lag_ts1, "FRIC_LAG_TS1", axisString);
        fric_max_u0=0.005;
        axisIniFile->Find(&fric_max_u0, "FRIC_MAX_U0", axisString);
        fric_max_u1=0.005;
        axisIniFile->Find(&fric_max_u1, "FRIC_MAX_U1", axisString);
        fric_type=0;
        axisIniFile->Find(&fric_type, "FRIC_TYPE", axisString);
        ...
        if (0 != emcAxisSetHomingParams(axis, home, offset, home_final_vel, search_vel,
                                        ...
                                        (int)is_shared, sequence, volatile_home, locking_indexer,pitchvalue,
                                        fric_delay_t,fric_lag_ts0,fric_lag_ts1,fric_max_u0,fric_max_u1,fric_type)) {
            if (EMC_DEBUG & EMC_DEBUG_CONFIG) {
        ...
        posfbflag = 0;
        axisIniFile->Find(&posfbflag,"USE_POSFBFLAG",axisString);
        printf("axis=%d   maxVelocity=%f\n",axis,maxVelocity);
        useabs = 0;
        axisIniFile->Find(&useabs,"USE_ABSPOS",axisString);

        if (0 != emcAxisSetMaxVelocity(axis, maxVelocity,posfbflag,useabs)) {
            if (EMC_DEBUG & EMC_DEBUG_CONFIG) {
