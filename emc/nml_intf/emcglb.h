...
#define MAXMINLEN 500

#define EMC_AXIS_MAX EMCMOT_MAX_AXIS
#define EMC_MAX_UWARNIO EMCMOT_MAX_UWARNIO
#define EMC_MAX_UINIO EMCMOT_MAX_UINIO
#define EMC_MAX_UOUTIO EMCMOT_MAX_UOUTIO
...
#define EMC_DEBUG_SENDPOINT     0X00000001
...
/*just used to keep track of unneccessary debug printing. */
    ...
    /*number of the line add to miniline once*/
    extern int maxlinearnum;

    extern int emcGetArgs(int argc, char *argv[]);
    extern void emcInitGlobals();

    extern int localEmcTrajAxes;
    extern unsigned int aux_which_z;
    extern double AxisOffset[9];     //record the offset of each axis,use in axis changing
