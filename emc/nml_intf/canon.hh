...
extern void SET_XY_ROTATION(double t, double offsetx, double offsety);
...
made. */
extern void SET_DOUBLEZ_CHANGE(int axis, bool flag, double *zdiff); // hong 2014-03-27  add a standard function to set whitch Z
//when in MDI or AUTO

void FLUSH_Z_OFFSET(int axis, double *zdiff) ; //HONG-20161107
void SET_M99_EXECUTE();
//hong-2014/04/15 exec the m99,when it executed,turn the exec status to reading
...
//hong use for indentify z_mode now;
void SET_LINEARAUX(int num,double val);
//use for get the G53 flag
void SET_G0_MODE(int val, int index);

void CLEAR_ENDPOINTS();
void SET_WAIT_SIGNAL(double delay,char *str);
void SET_ROTATION_DIS(double x = 0, double y = 0, double z = 0);
void SET_ROTATION_ZMODE(int zmode = 0);
void SET_MULTI_DEF(int type, int val = 0, double val2 = 0);
void SET_COOR_Z(double *x,double *y,double *z,int *gmode) ;
void SET_SYNCH_CMD();
...
*/
extern void SET_SINGLE_OFFSET_TOOL(int tool);
extern int GET_SINGLE_OFFSET_TOOL();
...
/* used for threading */
extern void START_SPEED_FEED_SYNCH(double feed_per_revolution, int velocity_mode);
extern void STOP_SPEED_FEED_SYNCH();


/* Machining Functions */
extern void THREE_2_CIRCLE(int lineno,double x,double y,double z,  \
                           double a,double b,double c,\
...
extern void MIN_STRAIGHT_FEED(int *line_number,double *minx,
...
extern void DWELL(double seconds,int spindleflag = 0);
...
extern void SET_SPINDLE_SPEED_INCANNON(double r);
...
extern void CHANGE_TOOL_NUMBER(int number, int compflag, double compval);
...
extern void G0_HOLD_ENABLE(int mode);
...
extern void SET_EXTERN_OUTPUT_BIT(int index,bool status);
