...
extern int reCaculatePos(EmcPose *pos, double *joints);

typedef struct KINEMATICDATA {

    int circle_type[EMCMOT_MAX_JOINTS];        //是否启用角度最短路径模式
    int circle_count[EMCMOT_MAX_JOINTS];   // 循环了多少圈
    float a_length;//A轴摆长
    int direct;//z轴方向加工补偿，1 向下为正，-1向上为负,0表示加工时z轴不动
    int method;//0计算弦长，保证眼睛xy投影与图纸一致，1计算弧长，图纸xy采用经纬度方式映射到球面上,2为调试模式，可以360度转
    double offset_x;//工件零点
    double offset_y;//工件零点
    double offset_z;//工件零点
    int flag;//初始化标志，1为初始化标志
...
extern struct KINEMATICDATA kinedata;
